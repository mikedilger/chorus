...
impl EventStore {
    ...
    pub fn get_event_by_offset(&self, offset: usize) -> Result<Option<Event>, Error> {
        // deserialize event
        let event = Event::read_from_buffer(&self.event_map[offset..])?;
        Ok(Some(event))
    }
    ...
    pub fn store_event(&self, event: &Event) -> Result<usize, Error> {
        let event_size = <Event as Writable<Endianness>>::bytes_needed(event)?;

        loop {
            let result = self
                .event_map
                .append(event_size, |dst| event.write_to_buffer(dst).unwrap());
